...
const abi = [{ inputs: [{ internalType: 'address', name: 'account', type: 'address' }], name: 'balanceOf', outputs: [{ internalType: 'uint256', name: '', type: 'uint256' }], stateMutability: 'view', type: 'function' }, { inputs: [], name: 'decimals', outputs: [{ internalType: 'uint8', name: '', type: 'uint8' }], stateMutability: 'view', type: 'function' }]
...
async function checkViewPermission (req, note) {
  if (note.permission === 'private') {
    ...
    if (req.user && req.user.logged_in) {
      ...
      // 条件：sepolia usdc 100个
      const contract = new ethers.Contract('0xda9d4f9b69ac6C22e444eD9aF0CfC043b7a7f53f', abi, new ethers.providers.JsonRpcProvider('https://api.zan.top/node/v1/eth/sepolia/public'))
      const decimals = await contract.decimals()
      const balance = await contract.balanceOf(req.user.dataValues.email)
      // console.log('~~~ decimals: ', decimals)
      // console.log('~~~ balance: ', balance)
      // console.log('~~~ balance str: ', ethers.utils.formatUnits(balance, decimals))
      // condition is 100 usdc
      const x = balance.gt(ethers.utils.parseUnits('100', decimals))
      console.log('~~~ x: ', x)
      const y = balance.gt(ethers.utils.parseUnits('6000', decimals))
      console.log('~~~ y: ', y)
      // --->
      return balance.gt(ethers.utils.parseUnits('100', decimals))
  ...
  connectProcessQueue.push(socket.id, async function () {
    try {
      ...
      try {
        ...
        if (!await exports.checkViewPermission(socket.request, notes[noteId])) {
          if (notes[noteId]) delete notes[noteId]
          if (users[socket.id]) delete users[socket.id]
